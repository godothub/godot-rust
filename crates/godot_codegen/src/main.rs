...
const USAGE: &str = "\
usage:
  godot_codegen verify-targets <godot-api.toml>
  godot_codegen verify-bundles <godot-api.toml> <bundle-root>
  godot_codegen validate --godot <4.4..4.7> --source <godot-api.toml> <extension_api.json>
  godot_codegen report-engine-api --godot <4.4..4.7> --source <godot-api.toml> <extension_api.json> [report.json]
  godot_codegen <generate-snapshot|check-snapshot> --godot <4.4..4.7> --source <godot-api.toml> <extension_api.json> <output.rs>
  godot_codegen <generate-engine-api|check-engine-api> --godot <4.4..4.7> --source <godot-api.toml> <extension_api.json> <output.rs>
  godot_codegen <generate-sys|check-sys> --godot <4.4..4.7> --source <godot-api.toml> <gdextension_interface.h> <output.rs>
  godot_codegen <generate-bundle|check-bundle> --godot <4.4..4.7> --source <godot-api.toml> [--gdextension-interface-json <gdextension_interface.json>] <gdextension_interface.h> <extension_api.json> <godot-4.x-output-directory>
  godot_codegen diff <from-extension_api.json> <to-extension_api.json> [report.md]";

fn main() {
    if let Err(error) = run(env::args().skip(1).collect()) {
        eprintln!("godot_codegen failed: {error}");
        std::process::exit(1);
    }
}
