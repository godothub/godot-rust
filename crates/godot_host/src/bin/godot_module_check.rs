...
fn main() {
    ...
    let Some(path) = arguments.next().map(PathBuf::from) else {
        eprintln!("usage: godot_module_check <project-module> [--exercise <res-path>]");
        std::process::exit(2);
    };
    ...
    match godot_host::validate_project_module(&path) {
        Ok(script_count) => {
    ...
    if let Some(source_path) = exercise_path {
        ...
        if let Err(error) = godot_host::exercise_project_module_callbacks(&path, &source_path) {
