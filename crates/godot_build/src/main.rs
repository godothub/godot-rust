...
fn main() {
    ...
    if let Some(cancel_file) = cancel_file {
        ...
        // process-local value.
        unsafe {
            std::env::set_var(godot_build::process_cancel_file_env(), cancel_file);
        }
    }
    let (request, response_file) = match arguments.as_slice() {
        ...
        [flag, request] if flag == "--request-hex" => {
            (Some(godot_build::decode_hex_request(request)), None)
        }
        ...
        {
            (
                Some(godot_build::decode_hex_request(request)),
                Some(PathBuf::from(response_file)),
            )
        }
        _ => {
            eprintln!(
                "usage: godot_build [--request <json> | --request-hex <hex>] \
    ...
    if let Some(request) = request {
        ...
        let response = godot_build::handle_json_request(&request);
    ...
    if let Err(error) = godot_build::serve(io::stdin().lock(), io::stdout().lock()) {
        eprintln!("godot-rust build daemon failed: {error}");
        std::process::exit(1);
    }
}

fn write_response_file(path: &Path, response: &godot_build::BuilddResponse) -> Result<(), String> {
